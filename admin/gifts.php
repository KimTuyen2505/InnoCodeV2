    ...
    if ($action === 'save_gift') {
        ...
        if (!empty($_FILES['image_file']['name'])) {
            ...
            if (!in_array($extension, $allowed, true)) {
                flash('error', 'Ảnh quà lưu niệm chỉ hỗ trợ JPG, PNG, WEBP hoặc GIF.');
                redirect('admin/gifts.php');
            }

            $uploadDir = dirname(__DIR__) . '/storage/uploads/gifts';
            if (!is_dir($uploadDir)) {
                mkdir($uploadDir, 0777, true);
            }

            $fileName = 'gift_' . date('YmdHis') . '_' . bin2hex(random_bytes(4)) . '.' . $extension;
            if (!move_uploaded_file($_FILES['image_file']['tmp_name'], $uploadDir . '/' . $fileName)) {
                flash('error', 'Không thể tải ảnh quà lưu niệm lên.');
                redirect('admin/gifts.php');
            }

            $imageUrl = APP_URL . '/storage/uploads/gifts/' . $fileName;
        }
...
<div class="row g-4">
    <div class="col-lg-4">
        ...
        <form class="bg-white rounded-2 p-4 shadow-sm mt-3" method="post" enctype="multipart/form-data">
    ...
    <div class="col-lg-8">
        <div class="bg-white rounded-2 shadow-sm table-responsive mb-4">
            <table class="table mb-0">
                <thead><tr><th>Quà</th><th>Ảnh</th><th>Tồn</th><th>Trạng thái</th><th>Sửa nhanh</th></tr></thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if (!empty($gift['image_url'])): ?>
                                <img class="admin-gift-preview" src="<?= e($gift['image_url']) ?>" alt="<?= e($gift['name']) ?>">
                            <?php else: ?>
                        ...
                        <td>
                            <form class="row g-2" method="post" enctype="multipart/form-data">
                                ...
                                <div class="col-md-4"><input class="form-control form-control-sm" name="name" value="<?= e($gift['name']) ?>" placeholder="Tên quà"></div>
                                ...
                                <div class="col-md-6"><input class="form-control form-control-sm" name="image_url" value="<?= e($gift['image_url']) ?>" placeholder="URL ảnh"></div>
                ...
                <?php endforeach; ?>
                <?php if (!$gifts): ?>
                    <tr><td colspan="5" class="text-muted">Chưa có quà lưu niệm.</td></tr>
                <?php endif; ?>
                </tbody>
            </table>
        </div>
