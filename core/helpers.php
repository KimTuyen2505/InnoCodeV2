...
function app_redirect_url(string $path): string
{
    ...
    $fragmentText = is_string($fragment) && $fragment !== '' ? '#' . $fragment : '';
    $pathWithoutFragment = explode('#', $path, 2)[0];
    $pathOnly = strtok($pathWithoutFragment, '?') ?: $pathWithoutFragment;

    if (!str_starts_with($pathWithoutFragment, 'admin/') && str_ends_with($pathOnly, '.php')) {
        $query = parse_url($pathWithoutFragment, PHP_URL_QUERY);
        $params = [];
        ...
        return url(page_from_path($pathWithoutFragment), $params) . $fragmentText;
    }

    return APP_URL . '/' . ltrim($pathWithoutFragment, '/') . $fragmentText;
}
...
{
    header('Location: ' . app_redirect_url($path));
    exit;
}
...
function piston_api_request(string $path, ?array $payload = null): array
{
    ...
    if ($status >= 400) {
        ...
        if (stripos($message, 'whitelist') !== false) {
            $message = 'Piston API public hiện yêu cầu whitelist. Hãy chạy Piston server riêng và cấu hình PISTON_API_BASE, ví dụ http://localhost:2000/api/v2.';
        }
